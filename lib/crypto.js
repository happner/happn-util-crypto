(function() {
  ...
  const bitcore = require('bitcore-lib'),
    bitcore_ecies = require('happn-bitcore-ecies'),
  ...
  function Crypto(opts) {
    ...
    this.sign = function(hash, privateKey, hashEncoding) {
      ...
      if (typeof hash === 'string') hash = new Buffer(hash, hashEncoding);
    ...
    this.validatePublicKey = function(publicKey, encoding) {
      try {
        ...
        if (encoding == 'base64') keyToCheck = convert(publicKey, 'base64', 'hex');
    ...
    this.validatePrivateKey = function(privateKey, encoding) {
      try {
        ...
        if (encoding == 'base64') keyToCheck = convert(privateKey, 'base64', 'hex');
